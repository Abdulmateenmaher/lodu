...
class _PathCell extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    if (index == 8) bg = kColors[1]!.main;
    if (index == 21) bg = kColors[2]!.main;
    if (index == 34) bg = kColors[3]!.main;
    if (index == 47) bg = kColors[0]!.main;
    ...
    // Arrow cells override
    if (index == 11) {
      content = Icon(
        Icons.chevron_right,
        size: cw * 0.85,
        color: kColors[1]!.main,
      );
      bg = Colors.white;
    } else if (index == 24) {
      content = Icon(
...
class _PiecesLayer extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return LayoutBuilder(
      builder: (context, constraints) {
        final size = constraints.maxWidth;
        return Stack(children: _buildPieces(game, size));
      },
    );
  }
  ...
  List<Widget> _buildPieces(GameNotifier game, double size) {
    ...
    groups.forEach((_, group) {
      for (int index = 0; index < group.length; index++) {
        ...
        // Selectable pieces go into a separate list rendered last (on top)
    ...
    // Render selectable pieces last so they appear on top and receive taps
